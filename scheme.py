...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        loc_frame=env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, loc_frame)
    elif isinstance(procedure, MuProcedure):
        return scheme_eval(procedure, env)
    else:
...
def apply_primitive(procedure, args, env):
    ...
    def make_args_lst(pr):
        if pr is nil:
            return
        args_lst.append(pr.first)
        make_args_lst(pr.second)
        
    make_args_lst(args)
    if procedure.use_env:
        args_lst.append(env)
    try:
        return procedure.fn(*args_lst)
    except TypeError:
        raise SchemeError
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        while formals:
            if vals is nil:
                raise SchemeError('More parameters than values')
            frame.define(formals[0], vals[0])
            formals, vals=formals.second, vals.second
        if vals is not nil:
            raise SchemeError('More values than parameters') 
        return frame
...
def do_lambda_form(vals, env):
    ...
    result = LambdaProcedure(formals, Pair('begin', vals.second), env)
    return result
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target[0]):
            check_form(target, 1)
            func=do_lambda_form(Pair(target.second, vals.second), env)
            env.define(target[0], func)
            return target[0]
    else:
...
def do_let_form(vals, env):
    ...
    while bindings is not nil:
        names, values=Pair(bindings.first.first,names), Pair(scheme_eval(bindings.first[1], env), values)
        ...
        if scheme_true(test):
            ...
            if body is not nil:
                if body.second is not nil:
                    return do_begin_form(body, env)
                return body.first
            return quote(scheme_eval(clause.first, env))
...
def check_formals(formals):
    ...
    while True:
        try:
            ...
            return SchemError(str(suspects[0]) + " is not defined in this frame")
        if scheme_atomp(suspects[0]) == False:
            return SchemeError(str(suspects[0]) + " is not a scheme atom")
        if suspects.second != nil and len(suspects.second) != 2:
            return SchemeError("not a well-formed list")
        if suspects.second == nil:
            return
        suspects = suspects.second
